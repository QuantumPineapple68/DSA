...
//        mazeObstaclesAllDir("",board,0,0);
        mazeObstaclesPrintAllDir("",board,0,0,path,1);
    }

    static void mazeObstaclesAllDir(String p, boolean[][] maze, int r, int c){
        ...
        if (r<maze.length-1) {
            mazeObstaclesAllDir(p+'D',maze,r + 1, c);
        }
        if (c<maze[0].length-1){
            mazeObstaclesAllDir(p+'R',maze, r,c+1);
        }
        if (r>0){
            mazeObstaclesAllDir(p+'U',maze, r-1,c);
        }
        if (c>0){
            mazeObstaclesAllDir(p+'L',maze, r,c-1);
        }
